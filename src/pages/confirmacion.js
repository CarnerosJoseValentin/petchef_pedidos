...
export default function Confirmacion() {
  ...
  const handleConfirmarPedido = async () => {
    ...
    try {
      ...
      // Validar y reducir stock AHORA que el pedido ya está persistido: la
      // Cloud Function deriva los ingredientes del propio documento del
      // pedido (no de un array armado acá), y valida que el pedido sea del
      // usuario autenticado.
      console.log("Validando y reduciendo stock...");
      try {
        const functions = getFunctions();
        const reducirStock = httpsCallable(functions, "reducirStockPedido");
        await reducirStock({ pedidoId });
        console.log("✅ Stock reducido correctamente");
      } catch (stockError) {
        console.error("Error con el stock:", stockError);
        // La propia Cloud Function ya marcó el pedido como
        // "cancelado_sin_stock" (usa Admin SDK, no depende de que el
        // cliente tenga permiso de escritura sobre pedidos ajenos/propios).
